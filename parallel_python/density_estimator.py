"""
Code from Vogels et al. for densitiy approximation 
    ...
    def rsample(
        ...
        for x in default_x:
            ...
            while not done:
                ...
                if iteration > 1000:
                    logger.error("Error creating samples in prior range, aborting")
                    low_n = bounds.get("low").detach().numpy()
                    low_h =  bounds.get("high").detach().numpy()
                    logger.error(f"Prior Range {low_n} {low_h}")
                    done = True
                    raise Exception
