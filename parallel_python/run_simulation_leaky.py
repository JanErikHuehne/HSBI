...
def simulation(sim_params, run_id, seed=None):
    ...
    R = 100*b2.Mohm
    eqs_neurons='''
                dv/dt=(-(v-V_reset)+I*R) / tau : volt (unless refractory)
                dI/dt = -I / (5*ms) : amp
                '''
    neurons = b2.NeuronGroup(NE+NI, model=eqs_neurons, threshold='v > V_th',
                             reset='v=V_reset', refractory=5*ms, method='exact')
    Pe = neurons[:NE]
    Pe.v= V_rest
    Pi = neurons[NE:]
    Pi.v = V_rest
    ...
    con_ee.w = 0.2
    # EI Plasticity
    con_ei = b2.Synapses(Pe, Pi, on_pre="I_post += 0.5*nA")
    con_ei.connect(p=epsilon,  condition='i != j')
   
    #  II Plasticity
    con_ii = b2.Synapses(Pi,Pi, on_pre="I_post -= 2*nA")
    ...
    S1 = b2.PoissonGroup(N=50,rates=input_freq*Hz)
    S2 = b2.PoissonGroup(N=25,rates=input_freq*Hz)
    poisson_syn_ee = b2.Synapses(S1, Pe, on_pre ='I_post += 0.2*nA')
    poisson_syn_ie = b2.Synapses(S2, Pi, on_pre ='I_post += 0.2*nA')
    poisson_syn_ee.connect(p=0.4)
    poisson_syn_ie.connect(p=0.4)
    ...
    if  (np.sum(last_ee[last_ee >= 16.0]) / len(last_ee)) > 0.05 or (np.sum(last_ie[last_ie >= 16.0]) / len(last_ie)) > 0.05:
        logger.info("Aborting after 10s  - Detecting too high EE or IE weights")
        logger.info(np.sum(last_ee[last_ee >= 16.0]))
        logger.info(np.sum(last_ie[last_ie >= 16.0]))
        #from matplotlib import pyplot as plt
        #plt.plot(np.mean(np.array(W_EE.w, dtype=float),axis=0), label='ee_mean')
        #plt.plot(np.mean(np.array(W_IE.w, dtype=float),axis=0), label='ie_mean')
        #plt.legend()
        #plt.savefig('/home/ge84yes/data/test_run/mean_ee.png')
        exit(0)
    elif  (np.sum(last_ee[last_ee <= 0.1]) / len(last_ee)) > 0.1 or (np.sum(last_ie[last_ie <= 0.1]) / len(last_ie)) > 0.1:
        logger.info("Aborting after 10s - Detecting too low EE or IE weights")
        exit(0)
    del W_IE
    del W_EE
    W_IE = b2.StateMonitor(con_ie, 'w', record=True, dt=4 * second)
    W_EE = b2.StateMonitor(con_ee, 'w', record=True, dt=4 * second)
    b2.run(10 * second)
    last_ee =  np.array(W_EE.w, dtype=float)[:, -1]
    last_ie =  np.array(W_IE.w, dtype=float)[:, -1]
    if  (np.sum(last_ee[last_ee >= 16.0]) / len(last_ee)) > 0.05 or (np.sum(last_ie[last_ie >= 16.0]) / len(last_ie)) > 0.05:
        ...
        logger.info("Aborting after 20s - Detecting too low EE or IE weights")
        exit(0)
    del W_IE
    del W_EE
    W_IE = b2.StateMonitor(con_ie, 'w', record=True, dt=4 * second)
    ...
    last_ie =  np.array(W_IE.w, dtype=float)[:, -1]
    if  (np.sum(last_ee[last_ee >= 16.0]) / len(last_ee)) > 0.05 or (np.sum(last_ie[last_ie >= 16.0]) / len(last_ie)) > 0.05:
        logger.info("Aborting after 30s - Detecting too high EE or IE weights")
        logger.info(np.sumlast_ee[last_ee >= 16.0])
        logger.info(np.sum(last_ie[last_ie >= 16.0]))
        exit(0)
    elif  (np.sum(last_ee[last_ee <= 0.1]) / len(last_ee)) > 0.1 or (np.sum(last_ie[last_ie <= 0.1]) / len(last_ie)) > 0.1:
        logger.info("Aborting after 30s - Detecting too low EE or IE weights")
        exit(0)
    del W_IE
    del W_EE
    W_IE = b2.StateMonitor(con_ie, 'w', record=True, dt=4 * second)
    ...
    last_ie =  np.array(W_IE.w, dtype=float)[:, -1]
    if  (np.sum(last_ee[last_ee >= 16.0]) / len(last_ee)) > 0.05 or (np.sum(last_ie[last_ie >= 16.0]) / len(last_ie)) > 0.05:
        logger.info("Aborting after 50s - Detecting too high EE or IE weights")
        exit(0)
    elif  (np.sum(last_ee[last_ee <= 0.1]) / len(last_ee)) > 0.1 or (np.sum(last_ie[last_ie <= 0.1]) / len(last_ie)) > 0.1:
        logger.info("Aborting  after 50s - Detecting too high EE or IE weights")
        exit(0)
